...
> ApiListResponseOfFeatureDetails get_feature_details_using_get(x_yapily_api_version=x_yapily_api_version)
...
with yapily.ApiClient(configuration) as api_client:
    ...
    try:
        # Retrieve details for Yapily's institution features
        api_response = api_instance.get_feature_details_using_get(x_yapily_api_version=x_yapily_api_version)
        pprint(api_response)
...
> Institution get_institution_using_get(institution_id, x_yapily_api_version=x_yapily_api_version)
...
x_yapily_api_version = 'x_yapily_api_version_example' # str | __Optional__. Determines the API version to use. Valid values are `1.0` or `2.0-ALPHA`. Defaults to `1.0` (optional)
    ...
    try:
        # Retrieves details of a specific institution available in Yapily
        api_response = api_instance.get_institution_using_get(institution_id, x_yapily_api_version=x_yapily_api_version)
        pprint(api_response)
    except ApiException as e:
...
> ApiListResponseOfInstitution get_institutions_using_get(x_yapily_api_version=x_yapily_api_version)
...
with yapily.ApiClient(configuration) as api_client:
    ...
    try:
        # Retrieves the list of institutions available in Yapily
        api_response = api_instance.get_institutions_using_get(x_yapily_api_version=x_yapily_api_version)
        pprint(api_response)
    except ApiException as e:
