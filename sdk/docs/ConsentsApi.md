...
> ApiResponseOfAuthorisationRequestResponse create_pre_authorisation_request_using_post(pre_authorisation_request)
...
with yapily.ApiClient(configuration) as api_client:
    ...
    try:
        # Initiate request for user to pre authorise
        api_response = api_instance.create_pre_authorisation_request_using_post(pre_authorisation_request)
        pprint(api_response)
    except ApiException as e:
